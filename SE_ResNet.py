...
class SE_Residual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, use_1x1_conv=False, stride=1,ratio=16):
        super().__init__()
        if use_1x1_conv:
        ...
        self.seblock=SE_Block(ratio,in_channels=out_channels)

    def forward(self,x):
        stack=self.model(x)
        se=self.seblock(stack)
        se=se+self.res(x)
        return F.relu(se)
...
def get_residual_block(num_res, in_channels, out_channels, down_first=False):
    ...
    for i in range(num_res):
        blk.append(SE_Residual(in_channels=in_channels, out_channels=out_channels, use_1x1_conv=(i==0 and down_first), stride=1+int(i==0 and down_first)))
        in_channels=out_channels
    return blk


def se_resnet(in_channels=3):
    b1=nn.Sequential(
